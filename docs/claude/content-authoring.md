...
## ⚠ Does the pack TEACH what its questions ask?
The mirror of the section below, and the one nothing checks. A question may only
use a technical term the pack itself introduces — in the chapter `notes`, the
`syllabus` prose, or inline in the stem.
- Measured 2026-09-23 on **g3eng-wrt-011**, reported by a parent as a wrong
  answer: *"Which sentence uses a SIMILE to describe something?"*, Grade 3,
  tagged **difficulty 1 Basic**. The answer was correct in the source, in the
  built bundle and in the live database, and the report's own captured meta
  showed the app served and graded the right string. **The word "simile"
  appeared nowhere else in `grade3-english`** — the Writing notes teach
  planning, statement/question/exclamation, commas in a list and "describing
  words and connectives", and simile is a **Grade 5** concept
  (`grade5-english` notes, `ch06_writing.js`, `exam_depth.js`). It was the only
  stem of the 40 in that chapter to use an unexplained technical term; the
  comparable item spells its own out — *"Which sentence adds SENSORY DETAIL
  (something you can hear)?"*.
- ⚠ **The child's record is the evidence, and it is unambiguous.** One attempt,
  wrong, `state: needs_practice`, and the report pressed **four seconds later**.
  She had picked *"There were very many stars in the night sky"* — a plain
  description, which is what a child who has never met the word does.
- ⚠ **"The answer is correct" does not close a wrong-answer report.** Three
  sources agreeing with each other says the pipeline is intact, not that the
  question is fair. Read what the pack teaches before replying to one.
- Replaced by `g3eng-wrt-076` (a describing-word question the chapter does
  teach) under a **NEW id**: ids carry progress and mistakes, so reusing one
  would silently re-point a child's history at a different question. The
  importer never deletes, so the old row, its orphaned
  `student_question_progress` row and its entry in the child's `DB.mistakes`
  blob all went by hand; the report was **resolved with an admin note, not
  deleted** — it is an audit record.
- ⚠ A deleted id left in `DB.mistakes` degrades quietly (`unresolved++`, the
  item is skipped) but if it is the child's ONLY due mistake the drill says
  *"Those questions could not be loaded. Check your connection"*, which blames
  the network for a content decision. Clean the blob when you withdraw a
  question.

## ⚠ The report card must say who reported, and who owns them
A `question_reports` row carries a `student_id` and a display-name snapshot and
nothing else, so an admin acting on one — replying, refunding, explaining —
had to hand-search the Members tab for a child whose name does not appear in
it. Both report surfaces now resolve
`students.family_id → families.parent_id → profiles.full_name`, with the
address through the **same `/api/admin-member-emails`** the Members tab uses
(`profiles` carries no email column and `auth.users` is not queryable from the
browser).
- ⚠ **All three reads work only because each SELECT policy carries
  `is_admin()`** — confirmed against live `pg_policies`, not the dump.
- ⚠ **`public.students` has COLUMN-LEVEL grants.** `family_id` is granted
  (checked live); a column that is not returns `42501`, which PostgREST turns
  into an **empty result, not an error** — every child would silently show as
  having no parent.
- ⚠ **A failed lookup caches NOTHING.** Caching a dropped connection as "no
  parent" blanks every row for the rest of the session. `undefined` (not looked
  up) and `null` (family genuinely gone) are deliberately different, and a row
  still in flight is left untouched rather than painted "no parent".
- ⚠ **Both surfaces, one painter.** The full Reports screen and the pending
  panel on the Questions tab render different markup; the painter matches on
  `[data-report-family]` so they cannot disagree, and the email span carries
  `data-member-email` so the existing painter fills it with no new wiring.
- ⚠ **"Reporter's comment" is not always a comment.** The comment box on the
  pupil's modal is optional and `submitReport()` sends `msg || reportType`, so
  a report filed with nothing typed arrives with its **type slug** as its
  message — printing it raw put `wrong_answer` directly under the
  "❌ Wrong answer" chip and read as a complaint nobody made. `_reportComment()`
  returns null for that case; text that merely *resembles* the label, typed by
  hand, still shows.
- ⚠ **The bank the card falls back to was pinned to grades 4, 5, 6 by literal**,
  written when those were the only live ones. Every grade 1–9 has been live
  since 2026-09-16, so a report on any other grade rendered *"is not in the
  question bank loaded here"* — which reads as a missing question rather than an
  unloaded grade. Derived from the packs now. ⚠ Deliberately **not**
  `GradeAccess.liveGrades()`: that also drops a grade the admin kill switch has
  taken down, and an admin triaging a report about that grade is exactly who
  still needs the question.
- ⚠⚠ **A reply an admin sent disappeared the moment it succeeded.**
  `sendAdminReply()` called `loadReports()`, which **rebuilds every card** and so
  empties `#report-thread-<id>` back to a bare "Load message thread" button. The
  card itself survives (`_reportsOpen` keeps it open), which is what made it read
  as *"the message was never saved"* rather than *"the panel was redrawn"*. It
  **was** saved, and the pupil had it the whole time: `get_student_reports`
  returns `reply_count` and `last_admin_message`, verified by running its inner
  query for the reporting child. `loadReports()` is now **awaited** and the
  thread repainted into the rebuilt card.
- ⚠ **The thread loads only on demand, so the button must say whether there IS
  one.** It read "Load message thread" whether there were five messages or none,
  so a reply already sent was indistinguishable from silence — the second half
  of the same bug. One batched count per page now labels it
  "💬 Show message thread (2)" or "No follow-up messages yet"; a report not yet
  counted keeps the neutral label rather than claiming there are none.
- ⚠⚠ **The child could WRITE to a thread they could not READ.**
  `add_report_message()` has always let a pupil post a follow-up — the textarea
  is right there in their inbox modal — and nothing ever read those messages
  back to them: `question_report_messages` is admin-only (a child has no JWT,
  they are `anon` + `x-student-token`), so the modal rendered
  `get_student_reports().last_admin_message`, **one** message. So a second admin
  reply **replaced** the first in the child's view — a follow-up correction
  silently overwrote the message it was correcting — and the child's own words
  were never shown to them at all. `_sendThreadReply()` then **closed the
  modal**, which is what made it read as "nothing happened".
  `get_student_report_thread(p_report_id, p_student_id)` is the read side that
  was missing (`migrations/20260923_student_report_thread.sql`).
  - ⚠ Its identity branch **mirrors `get_student_reports()` exactly**, parent
    preview included: a parent viewing the child's inbox has a JWT and no
    student token, and would otherwise get an empty thread under a summary they
    can already see.
  - ⚠ Ownership is checked on the **report row**, never trusted from the caller,
    and a report that is not theirs returns **empty rather than raising** — an
    error that fires only for rows that exist is an existence oracle, so "not
    yours" and "no messages" must answer identically.
  - ⚠ `GRANT ... TO anon` is load-bearing, not boilerplate: without it every
    child gets `42501` and the client turns that into an empty thread.
  - Verified against **production** over the real `anon` + `x-student-token`
    path (9 checks: the full thread, ownership isolation, an unknown id
    answering exactly as a non-owned one, and an expired session), using a
    3-minute session token that the harness deletes again.
- `scripts/test-student-report-thread.js` (22 checks) covers the render half.
- `scripts/test-report-parent-line.js` (33 checks). It **extracts each helper's
  source text from `engine/admin.js` and runs that**, because they live inside
  the `AdminPanel` IIFE and are unreachable from outside — a harness copy would
  be circular and would stay green while the app drifted. The extraction fails
  loudly if a function is renamed or duplicated.

## ⚠ Does the pack ASK what its syllabus teaches?
Every other content harness here counts questions. `audit-content-coverage.js`
flags a subsection under 20 items (⚠ across **every live pack** since
2026-09-19 — it used to hard-code grades 4–6 × five subjects and silently
skipped the other 31); `test-live-pack-content.js` asks whether each
declared chapter holds any; `test-exam-paper-shape.js` asks whether a whole paper
can be dealt. **None of them holds a list of what should be asked**, so all of
them are green on a pack that never mentions the water cycle.
- Measured on grade5-history, 2026-09-17, against a Grade 5 revision-notes PDF:
  **664 questions, every subsection above its minimum, every harness green — and
  68 facts from the notes with ZERO questions behind them**, plus 36 more with
  one or two. The whole of the water cycle, cyclonic rain, the leeward side, the
  prevailing winds, every part of climate change, and **Port Mathurin, the
  capital of Rodrigues**, appeared in no question at all.
- ⚠ **The gaps clustered in the SECOND HALF of each unit.** Each topic was opened
  properly and then the quota was met. That shape is what a count-driven batch
  leaves behind — and the filenames say so: `coverage_min5.js`, `topup_*.js`,
  `depth_hard.js`, `exam_depth.js` are all aimed at numbers, none at scope.
- `scripts/test-syllabus-facts.js` + one ledger per pack under
  `scripts/fact-ledgers/<pack>.json`. It is **step 5 of `preflight.js`**.
- ⚠ **It reads the question STEM AND ANSWER ONLY, never the explanation.** That
  distinction is the whole harness. A fact named inside an `explanation:` looks
  covered to anyone grepping and is a fact the child is never *asked*. The first
  two hand passes over grade5-history each missed a batch of gaps for exactly
  this reason, and the harness found **8 more** on the run after them — one of
  which (`cotton cloth and spices from India`) existed only as a **distractor**.
- ⚠ It reads **source, not the built bundles**, for the same reason
  `test-live-pack-content.js` does: the question is whether the content has been
  written, and that must not depend on whether someone rebuilt.
- ⚠ **A live pack with no ledger is reported, never failed.** Writing a ledger is
  real work; a harness that fails the build for 48 packs nobody has got to yet
  gets disabled, and then it protects nothing.
- ⚠ **A too-strict term is a false failure, and a false failure is how a harness
  gets ignored.** Two of the first eight were the ledger's fault, not the pack's
  (`"south-west indian ocean"` as one phrase, against an answer reading "The
  south-west"). Prefer `all: ["mascarene","south-west"]` over a long literal.
- ⚠ **The chapter's own `syllabus:` prose is not a safe ledger source.** Two
  grade5-history chapters understated their scope: `g5ge-weather` named no
  rainfall or climate-change content *while the real papers were already asking
  it* (`g5h-pp22-017`, `g5h-pp24-016`, `g5h-pp21-018`), and `port-louis` never
  mentioned Port Mathurin. Both were extended 2026-09-17. Build a ledger from the
  syllabus **document**, and check the past papers against it too.
...
- ⚠⚠ **That carry made a SECOND bug invisible for as long as it existed.**
  `qmFormChapterChange()` read `pack.SYLLABUS`. All 49 packs spell it
  `syllabus`, and `registerSubject()` normalises to that name, so **`.SYLLABUS`
  has never existed on any pack object** — the Subsection dropdown was empty for
  **every question in every pack**, and each question's own correct tag came
  back labelled *"— not declared in this chapter"*. It reads as content rot, and
  it invites an admin to "fix" a right tag by clearing it — which is exactly the
  invariant breach the carry exists to prevent. Found 2026-09-23 while checking
  a parent's report on `g3eng-wrt-011`, whose subsection is declared and always
  was (`test-subsection-invariant.js` was passing 45/45 throughout).
  ⚠ **There were TWO causes and fixing either alone still left it empty**: the
  key name, and the fact that `subjects/_index.js` is the LITE index carrying no
  syllabus at all, so `pack.syllabus` stays `{}` until `PackLoader.ensure()` has
  fetched the real manifest — which the question manager never called, unlike
  the admin syllabus viewer, which has done it all along.
  ⚠ The cascade is therefore **async**, and `qmOpenForm()` must **await** it:
  firing it and carrying the tag in straight afterwards lets the continuation
  wipe the `<select>` it had just been put into.
  `scripts/test-admin-subsection-picker.js` drives the real editor in Chrome
  (9 checks; 3 fail if the key name alone is reverted).
...
- ⚠ **`node scripts/preflight.js --import` used to refuse on a key that was
  present.** It checked `process.env.SUPABASE_SERVICE_ROLE_KEY` itself, but the
  only thing that ever read `.env` was the importer **one level down** — which
  the guard exited before reaching. So the command CLAUDE.md documents always
  answered *"SUPABASE_SERVICE_ROLE_KEY is not set"* while the key sat in `.env`
  correctly named, and the only way through was to inject the variable by hand.
  Fixed 2026-09-23 by reading `.env` **before** the guard; the guard itself was
  kept, because refusing before a minute of building is the right behaviour.
  ⚠ The reader is now **one shared module**, `scripts/lib/load-env.js`. It had
  been hand-written twice (here and `scripts/db-query.js`) and then not written
  a third time, which is the whole bug — and it carries a **CRLF trap** that has
  already cost this project a run: a `'\n'` split leaves a trailing `\r`, which
  is a regex line terminator, so a `$`-anchored pattern fails on every line and
  the file parses to `{}`. Every extra copy is another chance to get that wrong.
  `scripts/check.js` now fails on any file under `scripts/`, `netlify/` or
  `workers/` that assigns a computed `process.env[...]` key, and
  `scripts/test-env-loader.js` (19 checks) covers the parser and asserts on the
  SOURCE that preflight's require lands **before** its guard — a require after
  the guard is the same defect with an extra file.
