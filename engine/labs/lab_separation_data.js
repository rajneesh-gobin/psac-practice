...
const LabSeparationData = (() => {
  ...
  // mission questions asked WITH the notebook beside them, and the paper point.
  // `setup` tokens are applied silently before the Aim; `steps` are guide
  // tokens the child performs. An `ask` step lists every option that glows
  // and what a wrong one teaches - the bench HEARS a listed wrong option
  // without acting on it, so the runner's card is the lesson. A `wait:` step
  // glows the control that starts the run (🔥 Heat, 🫗 Pour, 🪟 Leave to dry…)
  // and then ⏩ Wait; `wait:point` is the crystallisation point.
  // ⚠ A paper is quoted only where this grade's own question files carry it:
  //   subjects/grade9-chemistry/questions/past_paper_2024.js holds Chemistry
  //   2024 Q2 (sea water, iodine and sand, the sublimation rig). Everything
  //   else says "In the exam you may be asked…".
  // ⚠ The "Which technique?" experiments start on the LAST mixture they need,
  //   because after a mixture is sorted the bench moves to the first unsorted
  //   one - so iodine_sand is followed by sea_water, salt_solution by
  //   muddy_water, and sea7 by iron, with no extra tap.
  const EXPERIMENTS = [
    // ── Grade 9 · C2 Mixtures & Separation Techniques ──
    { id: 'g9_two_errors', grades: [9], chapter: 'g9s-c2-mixtures', icon: '🔍',
      title: 'Can you spot the two errors in this rig?',
      aim: 'This distillation rig has two mistakes in it - the kind a paper asks you to find. Fix both, then run it and collect pure water.',
      setup: ['mode:distil', 'build:two_errors', 'goggles'],
      predict: { q: 'Look at the rig. Which two parts are wrong?', answer: 'ct',
        options: [{ id: 'ct', label: 'Condenser water and thermometer', sub: 'water in at the top; bulb in the liquid' },
                  { id: 'gb', label: 'Granules and bung', sub: 'no granules; receiver sealed' },
                  { id: 'fb', label: 'Flask and burner', sub: 'wrong flask; burner in the wrong place' }] },
      steps: [
        { ask: 'The cold water goes IN at the top. Which way should it go?', on: 'part:condenser:bottom', options: ['part:condenser:bottom', 'part:condenser:top'],
          wrong: { 'part:condenser:top': 'Water in at the top runs straight out of the lower outlet, so the jacket never fills. Steam passes through without condensing.' } },
        { ask: 'The thermometer bulb is down in the liquid. Where should it be?', on: 'part:thermo:arm', options: ['part:thermo:arm', 'part:thermo:liquid'],
          wrong: { 'part:thermo:liquid': 'Down in the liquid it reads the boiling salty water - above 100 °C and rising. It should read the vapour going into the condenser.' } },
        { on: 'wait:distillate', say: 'Both fixed. Tap 🔥 Heat, then ⏩ Wait. Watch the thermometer and the conical flask.' },
      ],
      see: { saw: 'The thermometer at the side arm read 100 °C and 25 cm³ of clear distillate collected in the conical flask. The salt stayed behind.',
             learn: 'Cold water goes IN at the bottom of the condenser so the jacket stays full, and the thermometer bulb sits level with the side arm, in the vapour.' },
      check: ['seawater:2', 'seawater:3', 'seawater:5'],
      exam: 'In the exam you may be given a drawing of a distillation rig and asked to find two errors. Check the condenser water, the thermometer bulb and the receiver.' },
    { id: 'g9_crystals', grades: [9], chapter: 'g9s-c2-mixtures', icon: '💎',
      title: 'How do you grow big blue crystals?',
      aim: 'A hot solution of copper(II) sulfate, boiled down to its crystallisation point. Test it, stop heating at the right moment, then cool it the right way.',
      setup: ['mode:crystal', 'goggles', 'heat', 'wait:point'],
      predict: { q: 'Which cooling gives the biggest crystals?', answer: 'slow',
        options: [{ id: 'slow', label: 'Leave it to cool slowly' }, { id: 'fast', label: 'Stand it in cold water' }, { id: 'dry', label: 'Keep heating until it is dry' }] },
      steps: [
        { on: 'rod', say: 'Tap 🥢 Glass rod. Do crystals form on the rod as the drop cools?' },
        { ask: 'Crystals on the rod - the crystallisation point. What now?', on: 'heat-off', options: ['heat-off', 'wait'],
          wrong: { wait: 'Keep heating and the basin boils dry: the solid spits out and the blue crystals turn into white powder. Stop at the crystallisation point.' } },
        { ask: 'Now cool it. For BIG crystals, which way?', on: 'cool:slow', options: ['cool:slow', 'cool:fast'],
          wrong: { 'cool:fast': 'Cooled quickly, lots of tiny crystals start at once and none has time to grow. Slow cooling grows a few big ones.' } },
        { on: 'wait:crystals', say: 'Tap ⏩ Wait and watch the basin as it cools.' },
      ],
      see: { saw: 'As the solution cooled slowly, a few large, well-shaped blue crystals of copper(II) sulfate grew in the basin.',
             learn: 'Copper(II) sulfate is far less soluble in cold water than in hot, so crystals come out as a hot saturated solution cools. Cool it slowly and they grow big.' },
      check: ['crystals:0', 'crystals:1', 'crystals:3'],
      exam: 'Chemistry 2024 Q2 names crystallisation as one of the three techniques. You may be asked how crystals are obtained from a solution: evaporate to the crystallisation point, then leave to cool.' },
    { id: 'g9_iodine', grades: [9], chapter: 'g9s-c2-mixtures', icon: '🟣',
      title: 'Which solid will sublime: iodine or sand?',
      aim: 'Grey-black iodine mixed with sand in an evaporating dish. Set it up safely, heat gently, and watch what climbs onto the cool funnel.',
      setup: ['mode:sublime', 'part:mixture:iodine', 'goggles'],
      predict: { q: 'Heat the mixture gently. What forms on the cool funnel?', answer: 'iodine',
        options: [{ id: 'iodine', label: 'Iodine crystals' }, { id: 'sand', label: 'Sand' }, { id: 'both', label: 'Both of them' }, { id: 'none', label: 'Nothing' }] },
      steps: [
        { ask: 'Iodine vapour is harmful. Where do you work?', on: 'part:place:hood', options: ['part:place:hood', 'part:place:bench'],
          wrong: { 'part:place:bench': 'On the open bench the violet vapour spreads into the room and irritates eyes, nose and lungs. Heat it in a fume cupboard.' } },
        { ask: 'What goes over the dish to catch the vapour?', on: 'part:cover:funnel_plug', options: ['part:cover:funnel_plug', 'part:cover:funnel', 'part:cover:none'],
          wrong: { 'part:cover:funnel': 'A funnel with an open stem catches some iodine, but a violet stream escapes up the stem. Plug it with cotton wool.',
                   'part:cover:none': 'With nothing over the dish the vapour rises away and nothing re-forms. It needs a cold surface - an inverted funnel.' } },
        { on: 'wait:sublimate', say: 'Tap 🔥 Heat gently, then ⏩ Wait. Watch the vapour - and the inside of the funnel.' },
      ],
      see: { saw: 'Violet vapour rose from the dish and shiny grey-black crystals of iodine formed on the cool inside of the funnel. The sand stayed in the dish.',
             learn: 'Iodine sublimes: solid straight to gas, and back to solid on a cold surface - the sublimate. Sand does not, so the two separate.' },
      check: ['sorter:2', 'sorter:3', 'sorter:4'],
      exam: 'Chemistry 2024 Q2: iodine is separated from sand by sublimation; label the rig (evaporating dish, tripod, sublimate); and an inverted funnel with a cotton wool plug stops the vapour escaping.' },
    { id: 'g9_which', grades: [9], chapter: 'g9s-c2-mixtures', icon: '🎯',
      title: 'Sea water or iodine: which technique?',
      aim: 'Two mixtures from the 2024 paper. For each one, pick the technique that keeps what you want - and the reason it works.',
      setup: ['mode:choose', 'mix:iodine_sand'],
      predict: { q: 'Which technique keeps the LIQUID and leaves the solid behind?', answer: 'distillation',
        options: [{ id: 'distillation', label: 'Distillation' }, { id: 'crystallisation', label: 'Crystallisation' }, { id: 'sublimation', label: 'Sublimation' }] },
      steps: [
        { ask: 'Iodine and sand - you want pure iodine. Which technique?', on: 'tech:sublimation', options: ['tech:sublimation', 'tech:distillation', 'tech:crystallisation', 'tech:filtration'],
          wrong: { 'tech:distillation': 'There is no liquid to boil off - these are two solids.',
                   'tech:crystallisation': 'Iodine barely dissolves in water, so there is no solution to crystallise from.',
                   'tech:filtration': 'Filtration needs a liquid to carry one solid through the paper. Iodine sublimes and sand does not - use that.' } },
        { ask: 'Why does sublimation work here?', on: 'why:sublimes', options: ['why:sublimes', 'why:bp', 'why:solubility', 'why:insoluble'],
          wrong: { 'why:bp': 'That is the idea behind distillation. Nothing here is a liquid with a boiling point to use.',
                   'why:solubility': 'That is the idea behind crystallisation. Iodine hardly dissolves, so solubility is no help.',
                   'why:insoluble': 'That is the idea behind filtration. There is no liquid to pour through a paper.' } },
        { ask: 'Sea water - you want pure water to drink. Which technique?', on: 'tech:distillation', options: ['tech:distillation', 'tech:crystallisation', 'tech:sublimation', 'tech:filtration'],
          wrong: { 'tech:crystallisation': 'Crystallisation keeps the salt and lets the water escape into the air - but it is the WATER you want.',
                   'tech:sublimation': 'Neither salt nor water sublimes, so there is nothing to catch on a cold surface.',
                   'tech:filtration': 'The salt is dissolved, so it passes straight through the filter paper. You would still have salty water.' } },
        { ask: 'Why does distillation work here?', on: 'why:bp', options: ['why:bp', 'why:solubility', 'why:sublimes', 'why:insoluble'],
          wrong: { 'why:solubility': 'That is crystallisation: it would keep the salt as crystals and lose the water.',
                   'why:sublimes': 'Neither salt nor water turns straight into a gas when heated.',
                   'why:insoluble': 'That is filtration. Dissolved salt passes straight through a filter paper.' } },
      ],
      see: { saw: 'Iodine and sand → sublimation, because iodine sublimes and sand does not. Sea water → distillation, because water boils at 100 °C and salt only at 1413 °C.',
             learn: 'To choose a technique, ask two things: what do I want to keep, and how are the two substances different - boiling point, solubility, or whether one sublimes?' },
      check: ['sorter:0', 'sorter:1', 'sorter:5'],
      exam: 'Chemistry 2024 Q2(a): the technique used to obtain (i) water from sea water - distillation; (ii) iodine from iodine and sand - sublimation.' },

    // ── Grade 8 · g8s-mixtures ──
    { id: 'g8_filter_mud', grades: [8], chapter: 'g8s-mixtures', icon: '🧻',
      title: 'Will the filter paper stop the mud?',
      aim: 'Muddy water, a filter funnel and a beaker. Set the filter up so that only clear water gets through - then pour and watch.',
      setup: ['mode:filter', 'part:fmix:muddy', 'part:fbeaker:sound'],
      predict: { q: 'Pour muddy water through the filter paper. What collects in the beaker?', answer: 'clear',
        options: [{ id: 'clear', label: 'Clear water' }, { id: 'muddy', label: 'Muddy water' }, { id: 'none', label: 'Nothing gets through' }] },
      steps: [
        { ask: 'The filter paper. Which one goes in the funnel?', on: 'part:paper:cone', options: ['part:paper:cone', 'part:paper:torn'],
          wrong: { 'part:paper:torn': 'Mud slips straight through the hole and the filtrate comes out cloudy. Use a whole paper, folded into a cone.' } },
        { ask: 'How do you pour the muddy water in?', on: 'part:pouring:rod', options: ['part:pouring:rod', 'part:pouring:fast'],
          wrong: { 'part:pouring:fast': 'Tipped in all at once, the liquid rises over the top of the paper and runs round it - mud gets into the beaker.' } },
        { on: 'wait:filtrate', say: 'Tap 🫗 Pour, then ⏩ Wait. Watch the paper and the beaker underneath.' },
      ],
      see: { saw: 'The mud stayed on the filter paper - the residue. 48 cm³ of clear water ran through into the beaker - the filtrate.',
             learn: 'Mud does not dissolve, and its particles are too big for the tiny holes in the paper. The solid left on the paper is the residue; the liquid that runs through is the filtrate.' },
      check: ['g8_clear:1', 'g8_clear:2', 'g8_clear:3'],
      exam: 'In the exam you may be asked to label a filtration diagram - residue and filtrate - and to say why a filtrate came out cloudy: a torn paper, or liquid over the top.' },
    { id: 'g8_salt_back', grades: [8], chapter: 'g8s-mixtures', icon: '💠',
      title: 'How do you get the salt back?',
      aim: 'Salt water in an evaporating basin, heated until the first crystals show at the edge. What do you do next - and what is left at the end?',
      setup: ['mode:evap', 'part:emix:salt', 'goggles', 'heat', 'wait:edge'],
      predict: { q: 'When all the water has gone, what is left in the basin?', answer: 'salt',
        options: [{ id: 'salt', label: 'White salt crystals' }, { id: 'nothing', label: 'Nothing - the salt went too' }, { id: 'black', label: 'A black solid' }] },
      steps: [
        { ask: 'Crystals at the edge of the liquid. What now?', on: 'heat-off', options: ['heat-off', 'wait'],
          wrong: { wait: 'Keep heating and the basin boils dry: hot salt spits out and some is lost. Stop at the first crystals and let the rest dry on its own.' } },
        { ask: 'The rest of the water still has to go. How?', on: 'leave', options: ['leave', 'heat'],
          wrong: { heat: 'Heating it dry makes hot salt spit out of the basin. Leave it somewhere warm: the water evaporates slowly and the crystals grow bigger.' } },
        { on: 'wait:dry', say: 'Tap ⏩ Wait and watch the salt crystals appear as it dries (time-lapse).' },
      ],
      see: { saw: 'All the water evaporated and 5 g of white salt crystals were left in the basin - the same salt that was dissolved.',
             learn: 'Evaporation: the water turns to vapour and escapes; the dissolved salt cannot, so it is left behind. Stop heating at the first crystals - never heat the basin dry.' },
      check: ['g8_sort:4',
        { q: 'Why is salt got back by evaporating the water, not by cooling the salt water?',
          options: ['Salt dissolves about as well in cold water as in hot', 'Salt cannot dissolve in hot water', 'Cooling makes the water evaporate faster', 'Cold water dissolves far more salt than hot'],
          why: '36 g of salt dissolves in 100 g of water at 20 °C and 39 g at 100 °C - cooling would bring hardly any out, so the water has to go.' }],
      exam: 'In the exam you may be asked how to obtain salt from salt water (evaporation) and why heating is stopped before the basin is dry.' },
    { id: 'g8_black_ink', grades: [8], chapter: 'g8s-mixtures', icon: '🌈',
      title: 'How many dyes are hiding in black ink?',
      aim: 'A spot of black ink on chromatography paper, water climbing up it. Set the paper up right, and count the colours that appear.',
      setup: ['mode:chroma', 'part:ink:black'],
      predict: { q: 'The water climbs past the black spot. What will you see?', answer: 'three',
        options: [{ id: 'three', label: 'Three coloured spots' }, { id: 'one', label: 'One black spot, higher up' }, { id: 'wash', label: 'The spot washes away' }] },
      steps: [
        { ask: 'Draw the start line. In what?', on: 'part:line:pencil', options: ['part:line:pencil', 'part:line:pen'],
          wrong: { 'part:line:pen': 'A felt-tip line dissolves and runs up the paper with your dyes - you cannot tell which colour came from where. Pencil does not dissolve.' } },
        { ask: 'Where must the water level be?', on: 'part:level:low', options: ['part:level:low', 'part:level:high'],
          wrong: { 'part:level:high': 'Above the start line, the ink spot dissolves straight into the water in the beaker. Nothing climbs the paper.' } },
        { on: 'wait:chromatogram', say: 'Tap 💧 Stand in water, then ⏩ Wait. Watch the water climb the paper - and the spot.' },
      ],
      see: { saw: 'The black spot separated into 3 spots - blue, red and yellow - at different heights. The blue dye travelled furthest.',
             learn: 'Black ink is a mixture of dyes. Each dye dissolves in the water by a different amount, so each is carried a different distance up the paper.' },
      check: ['g8_ink:0', 'g8_ink:2', 'g8_ink:3'],
      exam: 'In the exam you may be asked what a chromatogram with several spots shows (a mixture of dyes) and why the start line is drawn in pencil.' },
    { id: 'g8_which', grades: [8], chapter: 'g8s-mixtures', icon: '🧰',
      title: 'Which technique for which mixture?',
      aim: 'Salt water, then muddy water. For each, pick the technique that gets you what you want - and the reason it works.',
      setup: ['mode:choose', 'mix:salt_solution'],
      predict: { q: 'Salt water - you want the salt back. Which will work?', answer: 'evaporation',
        options: [{ id: 'evaporation', label: 'Evaporate the water' }, { id: 'filtration', label: 'Filter it' }, { id: 'decantation', label: 'Pour the water off (decant)' }] },
      steps: [
        { ask: 'Salt water - you want the salt. Which technique?', on: 'tech:evaporation', options: ['tech:evaporation', 'tech:filtration', 'tech:decantation', 'tech:magnet'],
          wrong: { 'tech:filtration': 'The salt is dissolved, so it passes straight through the filter paper with the water.',
                   'tech:decantation': 'Nothing settles: dissolved salt stays spread all through the water.',
                   'tech:magnet': 'Salt is not magnetic - a magnet picks up nothing.' } },
        { ask: 'Why does evaporation work here?', on: 'why:evaporates', options: ['why:evaporates', 'why:insoluble', 'why:settles', 'why:bp'],
          wrong: { 'why:insoluble': 'That is the idea behind filtration - but the salt IS dissolved, so no paper can catch it.',
                   'why:settles': 'That is decantation. Dissolved salt never settles to the bottom.',
                   'why:bp': 'That is distillation - it collects the water. You only want the salt, so just let the water go.' } },
        { ask: 'Muddy water - you want clear water. Which technique?', on: 'tech:filtration', options: ['tech:filtration', 'tech:decantation', 'tech:evaporation', 'tech:chromatography'],
          wrong: { 'tech:decantation': 'Fine mud stays floating in the water and does not settle quickly, so the water you pour off is still cloudy.',
                   'tech:evaporation': 'Evaporation would lose the water to the air and leave dried mud - but it is the water you want.',
                   'tech:chromatography': 'Chromatography separates the dyes in a colour. It cannot clear the mud out of a beaker of water.' } },
        { ask: 'Why does filtration work here?', on: 'why:insoluble', options: ['why:insoluble', 'why:settles', 'why:dyes', 'why:evaporates'],
          wrong: { 'why:settles': 'That is decantation - but fine mud floats and does not settle quickly.',
                   'why:dyes': 'That is chromatography. Mud is not a dye.',
                   'why:evaporates': 'That is evaporation - it would lose the water you want and leave dried mud.' } },
      ],
      see: { saw: 'Salt water → evaporation: the water evaporates and the dissolved salt is left behind. Muddy water → filtration: mud does not dissolve, so the paper traps it.',
             learn: 'Each technique uses one difference between the substances: does the solid dissolve, does it settle, is it magnetic, does the liquid boil off?' },
      check: ['g8_sort:2', 'g8_sort:1', 'g8_sort:0'],
      exam: 'In the exam you may be given a mixture and asked for the technique AND the property it uses - size, settling, evaporating, magnetism, or boiling point.' },

    // ── Grade 7 · g7s-elements ──
    { id: 'g7_mass', grades: [7], chapter: 'g7s-elements', icon: '⚖️',
      title: 'Does salt still weigh anything once it dissolves?',
      aim: 'A beaker of 100 g of water on a balance, and 5 g of salt. Tip the salt in, stir until it disappears, and keep an eye on the balance.',
      setup: ['mode:dissolve', 'part:solid:salt'],
      predict: { q: 'The salt dissolves and disappears. What will the balance read?', answer: '105',
        options: [{ id: '105', label: '105 g' }, { id: '100', label: '100 g' }, { id: '95', label: '95 g' }] },
      steps: [
        { ask: 'Which beaker of water goes on the balance?', on: 'part:dbeaker:sound', options: ['part:dbeaker:sound', 'part:dbeaker:cracked'],
          wrong: { 'part:dbeaker:cracked': 'A glass rod can knock a cracked beaker and split it - water and broken glass across the balance. A cracked beaker goes back to the teacher.' } },
        { on: 'add', say: 'Tap 🥄 Tip it in. Read the balance.' },
        { on: 'wait:dissolved', say: 'Tap 🥢 Stir, then ⏩ Wait. Watch the salt grains - and the balance.' },
      ],
      see: { saw: 'The balance read 105.0 g when the salt went in - and still 105.0 g after the salt had dissolved and disappeared.',
             learn: 'Dissolving does not destroy anything. The salt is still there, spread out between the water particles: 100 g + 5 g = 105 g of solution.' },
      check: ['g7_where:0', 'g7_where:2', 'g7_where:1'],
      exam: 'In the exam you may be asked what happens to the mass when a solid dissolves - nothing is lost, so 100 g of water + 5 g of salt = 105 g.' },
    { id: 'g7_salt_back', grades: [7], chapter: 'g7s-elements', icon: '💠',
      title: 'Can you get the salt back out of the water?',
      aim: 'Salt water in an evaporating basin over a burner. Heat it, stop at the right moment, then let it dry. Does the salt come back?',
      setup: ['mode:evap', 'part:emix:salt'],
      predict: { q: 'Evaporate all the water. What is left in the basin?', answer: 'salt',
        options: [{ id: 'salt', label: 'The same white salt' }, { id: 'nothing', label: 'Nothing at all' }, { id: 'black', label: 'A black solid' }] },
      steps: [
        { ask: 'Before you heat anything: what first?', on: 'goggles', options: ['goggles', 'heat'],
          wrong: { heat: 'A heated basin spits - hot drops fly out towards your face. Put on safety goggles before you heat anything.' } },
        { on: 'wait:edge', say: 'Tap 🔥 Heat, then ⏩ Wait for the first crystals at the edge of the liquid.' },
        { ask: 'Crystals at the edge. What now?', on: 'heat-off', options: ['heat-off', 'wait'],
          wrong: { wait: 'Keep heating and the last water boils away in bursts - hot salt spits out of the basin. Turn the heat off at the first crystals.' } },
        { on: 'wait:dry', say: 'Tap 🪟 Leave to dry, then ⏩ Wait. Watch the salt come back (time-lapse).' },
      ],
      see: { saw: 'The water evaporated and 5 g of white salt crystals were left in the basin - the same salt that was dissolved.',
             learn: 'Dissolving is a physical change: no new substance was made, and evaporating the water gets the salt back unchanged.' },
      check: ['g7_back:0', 'g7_back:1', 'g7_back:2'],
      exam: 'In the exam you may be asked whether dissolving is a physical or a chemical change - physical, because evaporating the water gets the salt back.' },
    { id: 'g7_water_back', grades: [7], chapter: 'g7s-elements', icon: '💧',
      title: 'Can you get the water back too?',
      aim: 'Sea water in a flask, ready to boil. Finish the rig, then turn the steam back into water and test what you collect.',
      setup: ['mode:distil', 'part:flask:sea', 'part:granules:yes', 'part:thermo:arm', 'part:receiver:open', 'part:burner:yes', 'goggles'],
      predict: { q: 'Boil the sea water and cool the steam. What drips out of the condenser?', answer: 'pure',
        options: [{ id: 'pure', label: 'Pure water' }, { id: 'salty', label: 'Salty water' }, { id: 'none', label: 'Nothing - the steam escapes' }] },
      steps: [
        { ask: 'The steam must cool back into water in the condenser. Which way does the cold water go?', on: 'part:condenser:bottom', options: ['part:condenser:bottom', 'part:condenser:top'],
          wrong: { 'part:condenser:top': 'In at the top, the water runs straight out at the bottom and the jacket never fills. The steam goes through without cooling.' } },
        { on: 'wait:distillate', say: 'Tap 🔥 Heat, then ⏩ Wait. Follow the steam into the condenser and watch the conical flask.' },
        { on: 'drop', say: 'Tap 💧 Test a drop of the water you collected. Does it leave anything behind?' },
      ],
      see: { saw: 'The sea water boiled, the steam cooled in the condenser, and 25 cm³ of clear water dripped into the conical flask. A drop of it dried and left nothing behind. The salt stayed in the flask.',
             learn: 'Distillation is two changes of state - liquid to gas (boiling), then gas to liquid (condensing). Both are physical changes, so you get pure water back AND the salt.' },
      check: [
        { q: 'Which process gets pure water out of sea water by boiling it and cooling the steam?',
          options: ['Distillation', 'Filtration', 'Dissolving', 'Decantation'],
          why: 'The water boils off as steam, the condenser cools it back to liquid, and the salt stays in the flask.' },
        { q: 'In the condenser the steam turns back into liquid water. What kind of change is that?',
          options: ['A physical change - condensing', 'A chemical change - a new substance is made', 'A chemical change - burning', 'A physical change - dissolving'],
          why: 'Gas to liquid is condensing, a change of state. No new substance is made, so it is a physical change.' },
        'g7_sort:3'],
      exam: 'In the exam you may be asked to name the process that gets pure water from sea water - distillation - and to say that boiling and condensing are physical changes.' },
    { id: 'g7_kinds', grades: [7], chapter: 'g7s-elements', icon: '🔹',
      title: 'Element, compound or mixture?',
      aim: 'Sea water from the lagoon, then a bar of iron. Sort each one - and say why it belongs where it does.',
      setup: ['mode:choose', 'mix:sea7'],
      predict: { q: 'Which of these is an element?', answer: 'iron',
        options: [{ id: 'iron', label: 'Iron' }, { id: 'sea', label: 'Sea water' }, { id: 'salt', label: 'Salt' }] },
      steps: [
        { ask: 'Sea water - water with salt dissolved in it. What is it?', on: 'tech:mixture', options: ['tech:mixture', 'tech:compound', 'tech:element'],
          wrong: { 'tech:compound': 'The salt is not joined to the water: evaporate the water and the salt is left behind.',
                   'tech:element': 'Sea water contains water and dissolved salts - more than one substance.' } },
        { ask: 'Why is sea water a mixture?', on: 'why:not_joined', options: ['why:not_joined', 'why:joined', 'why:one_atom'],
          wrong: { 'why:joined': 'That describes a compound. The salt is not chemically joined to the water - it can be evaporated apart.',
                   'why:one_atom': 'That describes an element. Sea water has water AND salt in it.' } },
        { ask: 'Iron - a grey metal, symbol Fe. What is it?', on: 'tech:element', options: ['tech:element', 'tech:compound', 'tech:mixture'],
          wrong: { 'tech:compound': 'Iron is made of iron atoms only (symbol Fe). A compound needs two or more different elements joined together.',
                   'tech:mixture': 'Pure iron is one substance on its own - nothing is mixed into it.' } },
        { ask: 'Why is iron an element?', on: 'why:one_atom', options: ['why:one_atom', 'why:joined', 'why:not_joined'],
          wrong: { 'why:joined': 'That describes a compound - two or more elements chemically joined. Iron has only one kind of atom.',
                   'why:not_joined': 'That describes a mixture. Nothing is mixed into pure iron.' } },
      ],
      see: { saw: 'Sea water → mixture: the salt and water are not chemically joined. Iron → element: it is made of iron atoms only.',
             learn: 'An element has one kind of atom. A compound has elements chemically joined. A mixture has substances that are not joined, so it can be separated by physical means.' },
      check: ['g7_sort:0', 'g7_sort:2', 'g7_sort:1'],
      exam: 'In the exam you may be asked which of a list is a mixture, and how a mixture differs from a compound.' },
  ];
  ...
  return { ROOM, TIME_LAPSE, POINTS, STEAM_PER_CM3, FLASKS, DISTIL, PARTS, PART_ORDER, ERROR_ORDER, checkRig, RIGS, liquidBoils,
           ...
           TECH8, PRIN8, TP8, MIX8, KIND7, PRIN7, KP7, MIX7, CHOOSE, DONE_DISC, DRY_CARD, FACTS_LOW, PIC, EXPERIMENTS };
})();
