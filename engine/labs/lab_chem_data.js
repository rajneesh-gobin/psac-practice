...
const LabChem = (() => {
  ...
  // One question a child can say back, the tube already set up, a tap to
  // predict, at most five real decisions, what they saw, two or three
  ...
  // tokens the child performs (an `ask` step lists every option that glows and
  // what a wrong one teaches - a listed wrong is HEARD by the bench, never
  // acted, so potassium never meets acid and nothing is ever tasted).
  // `check` refs are "<mission id>:<quiz index>" or an inline question whose
  // FIRST option is the answer. Extra tokens the bench accepts for these:
  //   dropper:<base>  a fresh tube of 5 cm³ hydrochloric acid + indicator and
  //                   the dropper (setup only); drop:1|5|10|acid  the dropper;
  //   neutral         fires when the dropper lands on pH 7;
  //   evap            the Grade 9 "Evaporate" tool (a neutral salt solution
  //                   leaves crystals); taste / bung  only ever listed as wrong.
  // ⚠ Every See text is replayed through pH()/pHMix()/reaction() by
  //   scripts/test-labs-chem-data.js; change the numbers here, not the prose.
  const EXPERIMENTS = [
    // ── Grade 9 · g9s-c4-metals ──
    { id: 'g9_fastest', grades: [9], chapter: 'g9s-c4-metals', icon: '🏁',
      title: 'Which metal fizzes fastest?',
      aim: 'Zinc is already fizzing steadily in dilute hydrochloric acid. Test magnesium the same way and compare.',
      setup: ['goggles', 'liquid:hcl', 'metal:zinc', 'observe'],
      predict: { q: 'Magnesium is above zinc in the reactivity series. How will it fizz?', answer: 'faster',
        options: [{ id: 'faster', label: 'Faster than zinc', sub: 'vigorous fizzing' }, { id: 'same', label: 'The same as zinc' }, { id: 'slower', label: 'Slower than zinc' }, { id: 'none', label: 'Not at all' }] },
      steps: [
        { ask: 'Zinc is done. To test magnesium fairly, what first?', on: 'rinse', options: ['rinse', 'metal:magnesium'],
          wrong: { 'metal:magnesium': 'Two metals in one tube is not a fair test, and the zinc has already used up some of the acid. Empty & rinse, then fresh acid.' } },
        { ask: 'Clean tube. Acid first, or metal first?', on: 'liquid:hcl', options: ['liquid:hcl', 'metal:magnesium'],
          wrong: { 'metal:magnesium': 'Metal in an empty tube does nothing - and pouring acid onto it afterwards can splash. Acid first, then the metal.' } },
        { ask: 'Same acid, same amount. Now add a metal ABOVE zinc in the series.', on: 'metal:magnesium', options: ['metal:magnesium', 'metal:copper'],
          wrong: { 'metal:copper': 'Copper is at the very bottom of the series, below hydrogen. It would not fizz at all.' } },
        { on: 'observe', say: 'Watch the tube. Faster or slower than the zinc was?' },
        { ask: 'Test the gas. Which splint shows hydrogen?', on: 'pop', options: ['pop', 'glow'],
          wrong: { glow: 'A glowing splint relights only in OXYGEN. For hydrogen you need a lighted splint: it burns with a squeaky pop.' } },
      ],
      see: { saw: 'Magnesium fizzed vigorously - much faster than the zinc. The lighted splint gave a squeaky pop: hydrogen.',
             learn: 'The higher a metal is in the reactivity series, the faster it reacts with acid. Metal + acid → salt + hydrogen, and the fizzing is the hydrogen.' },
      check: ['race:1', 'race:2', 'race:4'],
      exam: 'NCE 2024 (g9s-pp24c-007): which gas is produced when zinc reacts with dilute hydrochloric acid? Hydrogen - it burns with a squeaky pop.' },
    { id: 'g9_copper', grades: [9], chapter: 'g9s-c4-metals', icon: '🟤',
      title: 'Will copper fizz in acid?',
      aim: 'Copper sits below hydrogen in the reactivity series. Drop it into dilute hydrochloric acid and see whether anything at all happens.',
      setup: ['goggles', 'liquid:hcl'],
      predict: { q: 'Copper in dilute hydrochloric acid. What will you see?', answer: 'none',
        options: [{ id: 'none', label: 'Nothing - not one bubble' }, { id: 'slow', label: 'A few slow bubbles' }, { id: 'fast', label: 'Vigorous fizzing' }, { id: 'colour', label: 'The acid turns green' }] },
      steps: [
        { ask: 'Which metal is BELOW hydrogen in the reactivity series?', on: 'metal:copper', options: ['metal:copper', 'metal:zinc', 'metal:magnesium'],
          wrong: { 'metal:zinc': 'Zinc is above hydrogen - it fizzes steadily. Look at the very bottom of the series.',
                   'metal:magnesium': 'Magnesium is well above hydrogen and fizzes vigorously. Look at the very bottom of the series.' } },
        { on: 'observe', say: 'Watch the tube closely for a few seconds. Any bubbles at all?' },
        { ask: 'Nothing. Is the acid the problem? Add a metal ABOVE hydrogen to the same tube.', on: 'metal:zinc', options: ['metal:zinc', 'metal:copper'],
          wrong: { 'metal:copper': 'More copper changes nothing - it still cannot push hydrogen out of the acid. Try a metal above hydrogen.' } },
        { on: 'observe', say: 'Watch the tube. Does the zinc fizz in the acid the copper refused?' },
      ],
      see: { saw: 'Copper: not a single bubble. Zinc dropped into the same acid fizzed steadily, so the acid was fine. Copper just cannot react.',
             learn: 'Only a metal above hydrogen in the reactivity series can push hydrogen out of an acid. Copper is below hydrogen, so with acid it does nothing.' },
      check: [
        { q: 'Why did copper not react with the dilute acid?',
          options: ['It is below hydrogen in the reactivity series', 'The acid was too weak', 'Copper only reacts with sulfuric acid', 'The tube was too cold'],
          why: 'Copper sits below hydrogen, so it cannot push hydrogen out of an acid. The same acid made zinc fizz, so the acid was fine.' },
        { q: 'Which metals CAN push hydrogen out of an acid?',
          options: ['Metals above hydrogen in the reactivity series', 'Metals below hydrogen in the reactivity series', 'Only the alkali metals', 'Every metal'],
          why: 'Magnesium, zinc and iron are above hydrogen and fizz in acid; copper is below and does nothing.' },
      ],
      exam: 'NCE 2024 (g9s-pp24c-029): metal Y reacts with neither cold water nor steam - copper, the least reactive metal here. In dilute acid it gives no bubbles either.' },
    { id: 'g9_sodium', grades: [9], chapter: 'g9s-c4-metals', icon: '🏃',
      title: 'What does sodium do in water?',
      aim: 'A tube of water, a safety screen, and your teacher holding a piece of sodium the size of a grain of rice. Watch what happens.',
      setup: ['liquid:water'],
      predict: { q: 'What will the sodium do in the water?', answer: 'float',
        options: [{ id: 'float', label: 'Float, whizz about and fizz' }, { id: 'sink', label: 'Sink and stay still' }, { id: 'dissolve', label: 'Dissolve with nothing to see' }, { id: 'black', label: 'Turn black' }] },
      steps: [
        { ask: 'The tube holds water. Sodium is an alkali metal. What goes in?', on: 'metal:sodium', options: ['metal:sodium', 'liquid:hcl'],
          wrong: { 'liquid:hcl': 'Never acid. Sodium in acid reacts explosively. An alkali metal only ever meets WATER, in a tiny piece, behind a safety screen.' } },
        { on: 'demo-end', say: 'Watch the tube until the sodium has all gone. Does it float or sink?' },
        { ask: 'The water looks the same. Has it changed? Test it.', on: 'liquid:indicator', options: ['liquid:indicator', 'liquid:hcl'],
          wrong: { 'liquid:hcl': 'Acid would cancel out whatever the sodium made - and then you could not see it. Universal indicator shows the pH without changing it.' } },
      ],
      see: { saw: 'The sodium floated, melted into a ball and whizzed about fizzing until it was gone. Universal indicator then turned purple: the water is now alkaline.',
             learn: 'Sodium + water → sodium hydroxide + hydrogen. Sodium floats because it is less dense than water, and the sodium hydroxide it makes is a strong alkali.' },
      check: [
        { q: 'Sodium reacts with water to form sodium hydroxide and a gas. Which gas?',
          options: ['Hydrogen', 'Oxygen', 'Carbon dioxide', 'Chlorine'],
          why: 'Metal + water → metal hydroxide + hydrogen. With sodium it is fast enough to send the sodium fizzing across the surface.' },
        { q: 'Why did the sodium float on the water?',
          options: ['It is less dense than water', 'The hydrogen held it up', 'It was too hot to sink', 'Sodium cannot get wet'],
          why: 'Sodium is less dense than water, so it floats - and it darts about as the hydrogen it makes pushes it along.' },
        { q: 'After the demo, universal indicator turned purple. Why?',
          options: ['Sodium hydroxide, a strong alkali, was made', 'The sodium dissolved and coloured the water', 'Hydrogen turns indicator purple', 'The water got hot'],
          why: 'Sodium + water → sodium hydroxide + hydrogen. Sodium hydroxide is a strong alkali, so universal indicator goes purple (pH 12 to 14).' },
      ],
      exam: 'NCE 2024 (g9s-pp24c-025, -026): sodium reacts with water to form sodium hydroxide and hydrogen; it floats and moves rapidly on the surface, fizzing.' },
    // ── Grade 9 · g9s-c5-salts ──
    { id: 'g9_salt', grades: [9], chapter: 'g9s-c5-salts', icon: '🧂',
      title: 'Can we make salt from an acid?',
      aim: 'Hydrochloric acid with universal indicator, 17 drops of sodium hydroxide already in: still red. Take it to exactly pH 7, then get the salt out.',
      setup: ['goggles', 'dropper:naoh', 'drop:10', 'drop:5', 'drop:1', 'drop:1'],
      predict: { q: 'Neutralise it, then evaporate the water. What is left in the tube?', answer: 'nacl',
        options: [{ id: 'nacl', label: 'White crystals of sodium chloride' }, { id: 'nothing', label: 'Nothing at all' }, { id: 'naoh', label: 'Sodium hydroxide' }, { id: 'hcl', label: 'Hydrochloric acid' }] },
      steps: [
        { ask: 'Near the end a drop flashes green where it lands. How much now?', on: 'drop:1', options: ['drop:1', 'drop:5'],
          wrong: { 'drop:5': 'Five drops would shoot straight past pH 7 to purple - alkaline. Near the end point, one drop at a time.' } },
        { on: 'drop:1', say: 'Still red. 💧 Add 1 drop more and swirl.' },
        { on: 'neutral', say: '💧 Add 1 drop at a time until it turns green: pH 7.' },
        { ask: 'Green, pH 7: sodium chloride solution. How do you get the salt out?', on: 'evap', options: ['evap', 'rinse', 'pop'],
          wrong: { rinse: 'That pours your salt down the sink! It is dissolved in the water - you need the water gone, not the tube emptied.',
                   pop: 'Nothing is fizzing, so there is no gas to test. The salt is dissolved in the water.' } },
      ],
      see: { saw: 'One drop at a time, the indicator went from red to green: pH 7. Evaporating the water left white crystals of sodium chloride.',
             learn: 'Acid + alkali → salt + water: hydrochloric acid + sodium hydroxide → sodium chloride + water. Evaporate the water and the salt is left behind.' },
      check: ['neutral:0', 'neutral:2', 'neutral:3'],
      exam: 'NCE 2024 (g9s-pp24c-028, -030): sodium hydroxide + acid is neutralisation, and the salt it makes here, sodium chloride, is the one that seasons food.' },
    { id: 'g9_sulfate', grades: [9], chapter: 'g9s-c5-salts', icon: '🧪',
      title: 'Which salt does sulfuric acid make?',
      aim: 'Every acid makes its own family of salts. Put magnesium into dilute sulfuric acid and see which salt the notebook names.',
      setup: ['goggles'],
      predict: { q: 'Magnesium + sulfuric acid → ? + hydrogen', answer: 'sulfate',
        options: [{ id: 'sulfate', label: 'Magnesium sulfate' }, { id: 'chloride', label: 'Magnesium chloride' }, { id: 'sulfide', label: 'Magnesium sulfide' }, { id: 'oxide', label: 'Magnesium oxide' }] },
      steps: [
        { ask: 'Which acid makes a SULFATE?', on: 'liquid:h2so4', options: ['liquid:h2so4', 'liquid:hcl'],
          wrong: { 'liquid:hcl': 'Hydrochloric acid makes CHLORIDES. Sulfates come from sulfuric acid.' } },
        { ask: 'Now a metal that reacts with acid.', on: 'metal:magnesium', options: ['metal:magnesium', 'metal:copper'],
          wrong: { 'metal:copper': 'Copper is below hydrogen in the reactivity series - it cannot react with an acid. Pick a metal above hydrogen.' } },
        { on: 'observe', say: 'Watch the tube. What is the fizzing?' },
        { ask: 'Prove the gas is hydrogen. Which test?', on: 'pop', options: ['pop', 'glow'],
          wrong: { glow: 'A glowing splint relights only in oxygen. Hydrogen is tested with a LIGHTED splint - a squeaky pop.' } },
      ],
      see: { saw: 'Magnesium fizzed hard in sulfuric acid and the lighted splint popped. The notebook wrote: magnesium + sulfuric acid → magnesium sulfate + hydrogen.',
             learn: 'Metal + acid → salt + hydrogen. The acid decides the salt: sulfuric acid makes sulfates, hydrochloric acid makes chlorides.' },
      check: ['race:3', 'race:1', 'race:2'],
      exam: 'NCE (g9s-c5-006, -007): hydrochloric acid gives sodium chloride, sulfuric acid gives potassium sulfate - the metal comes from the base or metal, the rest from the acid.' },

    // ── Grade 8 · g8s-acids ──
    { id: 'g8_lemon', grades: [8], chapter: 'g8s-acids', icon: '🍋',
      title: 'Is lemon juice an acid?',
      aim: 'Lemon juice from the kitchen is in the tube. Find out whether it is an acid - without tasting it.',
      setup: ['liquid:lemon'],
      predict: { q: 'Add universal indicator to lemon juice. What colour?', answer: 'red',
        options: [{ id: 'red', label: 'Red', sub: 'an acid' }, { id: 'green', label: 'Green', sub: 'neutral' }, { id: 'purple', label: 'Purple', sub: 'an alkali' }] },
      steps: [
        { ask: 'How do you find out if it is an acid?', on: 'liquid:indicator', options: ['liquid:indicator', 'taste', 'litmus:red'],
          wrong: { taste: 'Never taste anything in a lab - many chemicals are toxic or corrosive, and a tube may hold something from before. Use an indicator.',
                   'litmus:red': 'Red litmus only changes colour in an ALKALI. In an acid it stays red and tells you nothing. Universal indicator gives the pH.' } },
        { ask: 'Now check with litmus paper. Which one changes in an acid?', on: 'litmus:blue', options: ['litmus:blue', 'litmus:red'],
          wrong: { 'litmus:red': 'Red litmus stays red in an acid. It is BLUE litmus that an acid turns red.' } },
      ],
      see: { saw: 'Universal indicator turned red: pH 2. Blue litmus paper turned red too. Lemon juice is an acid.',
             learn: 'An acid turns universal indicator red or orange (pH below 7) and turns blue litmus red. Acids taste sour - but in a lab you never taste anything.' },
      check: ['g8_survey:0', 'g8_survey:4',
        { q: 'Blue litmus paper turned red in the lemon juice. What does that show?',
          options: ['Lemon juice is an acid', 'Lemon juice is an alkali', 'Lemon juice is neutral', 'The paper was wet'],
          why: 'An acid turns blue litmus red. (Acid - Red.) An alkali would turn red litmus blue instead.' }],
      exam: 'Grade 8 (g8s-acids-001): what colour does litmus turn in an acidic solution? Red.' },
    { id: 'g8_litmus', grades: [8], chapter: 'g8s-acids', icon: '🟥',
      title: 'Which litmus paper changes colour?',
      aim: 'Vinegar in the tube, and two litmus papers: red and blue. Only one of them will change in an acid. Then try an alkali.',
      setup: ['liquid:vinegar'],
      predict: { q: 'Vinegar is an acid. Which litmus paper will change colour?', answer: 'blue',
        options: [{ id: 'blue', label: 'Blue turns red' }, { id: 'red', label: 'Red turns blue' }, { id: 'both', label: 'Both change' }, { id: 'neither', label: 'Neither changes' }] },
      steps: [
        { on: 'litmus:blue', say: 'Dip the 🟦 Blue litmus paper in the vinegar.' },
        { on: 'litmus:red', say: 'Now dip the 🟥 Red litmus paper. Does it change?' },
        { ask: 'Now an alkali: toothpaste. What first?', on: 'rinse', options: ['rinse', 'liquid:toothpaste'],
          wrong: { 'liquid:toothpaste': 'Toothpaste poured into vinegar would neutralise it. A mixture tells you nothing about either one - use a clean tube.' } },
        { on: 'liquid:toothpaste', say: 'Pour in the 🧪 Toothpaste - it is mixed with water.' },
        { ask: 'Which paper will the alkali change?', on: 'litmus:red', options: ['litmus:red', 'litmus:blue'],
          wrong: { 'litmus:blue': 'Blue litmus only changes in an ACID. An alkali leaves it blue - it is the red paper that turns.' } },
      ],
      see: { saw: 'In vinegar, blue litmus turned red and red litmus stayed red. In toothpaste, red litmus turned blue.',
             learn: 'An acid turns blue litmus red. An alkali turns red litmus blue. Litmus says acid or alkali - but not how strong.' },
      check: [
        { q: 'What does an acid do to litmus paper?',
          options: ['Turns blue litmus red', 'Turns red litmus blue', 'Turns blue litmus green', 'Bleaches both papers white'],
          why: 'Acid - Red: an acid turns blue litmus red. An alkali turns red litmus blue.' },
        { q: 'Red litmus stayed red in the vinegar. On its own, what does that tell you?',
          options: ['Vinegar is not an alkali', 'Vinegar is an acid', 'Vinegar is neutral', 'The paper was faulty'],
          why: 'Red litmus only changes in an alkali. Staying red rules out an alkali - it could still be an acid or neutral, so the blue paper was needed too.' },
        'g8_survey:4'],
      exam: 'Grade 8 (g8s-acids-014): what colour does red litmus paper turn in an alkaline solution? Blue.' },
    { id: 'g8_neutral', grades: [8], chapter: 'g8s-acids', icon: '💊',
      title: 'Can we make it neutral?',
      aim: 'Hydrochloric acid with universal indicator: red, like an upset stomach. 17 drops of antacid are already in. Finish the job, one drop at a time.',
      setup: ['goggles', 'dropper:antacid', 'drop:10', 'drop:5', 'drop:1', 'drop:1'],
      predict: { q: 'When the acid is exactly neutral, what colour will the indicator be?', answer: 'green',
        options: [{ id: 'green', label: 'Green', sub: 'pH 7' }, { id: 'red', label: 'Red' }, { id: 'purple', label: 'Purple' }, { id: 'yellow', label: 'Yellow' }] },
      steps: [
        { ask: 'A drop now flashes green where it lands. How much antacid next?', on: 'drop:1', options: ['drop:1', 'drop:5'],
          wrong: { 'drop:5': 'Five drops at once would take it past neutral to blue - alkaline. Near the end, add one drop at a time and swirl.' } },
        { on: 'drop:1', say: 'Still red. 💧 Add 1 drop more, and swirl.' },
        { on: 'neutral', say: '💧 Add 1 drop at a time. Stop the moment it turns green.' },
      ],
      see: { saw: 'Drop by drop, the antacid turned the acid from red to green: pH 7, exactly neutral.',
             learn: 'An antacid is a base. Acid + base → salt + water: this is neutralisation, and it is how an antacid settles too much stomach acid.' },
      check: ['g8_antacid:0', 'g8_antacid:1', 'g8_antacid:2'],
      exam: 'Grade 8 (g8s-acids-005): what is neutralisation? An acid and a base reacting to form a salt (and water).' },
    { id: 'g8_hydrogen', grades: [8], chapter: 'g8s-acids', icon: '💥',
      title: 'What gas does a metal make in acid?',
      aim: 'Dilute hydrochloric acid is in the tube. Add magnesium, watch it fizz, then prove which gas it is.',
      setup: ['goggles', 'liquid:hcl'],
      predict: { q: 'Magnesium in acid. Which gas fizzes off?', answer: 'h2',
        options: [{ id: 'h2', label: 'Hydrogen' }, { id: 'co2', label: 'Carbon dioxide' }, { id: 'o2', label: 'Oxygen' }, { id: 'none', label: 'No gas at all' }] },
      steps: [
        { ask: 'Which solid is a METAL?', on: 'solid:magnesium', options: ['solid:magnesium', 'solid:marble'],
          wrong: { 'solid:marble': 'Marble is calcium carbonate, not a metal - with acid it gives carbon dioxide. Pick the metal.' } },
        { on: 'observe', say: 'Watch the tube. How hard does it fizz?' },
        { ask: 'Test the gas. Which test is for hydrogen?', on: 'pop', options: ['pop', 'lime'],
          wrong: { lime: 'Limewater tests for carbon dioxide - with hydrogen it stays clear. Hydrogen is tested with a lighted splint.' } },
      ],
      see: { saw: 'Magnesium fizzed hard in the acid. The lighted splint gave a squeaky pop: the gas is hydrogen.',
             learn: 'Metal + acid → salt + hydrogen. Hydrogen burns with a squeaky pop when a lighted splint is held at the mouth of the tube.' },
      check: [
        { q: 'Which gas is made when an acid reacts with a metal such as magnesium?',
          options: ['Hydrogen', 'Carbon dioxide', 'Oxygen', 'Chlorine'],
          why: 'Metal + acid → salt + hydrogen. The squeaky pop proved it.' },
        { q: 'How do you test for hydrogen?',
          options: ['A lighted splint gives a squeaky pop', 'Limewater turns milky', 'A glowing splint relights', 'Blue litmus turns red'],
          why: 'Hydrogen burns with a squeaky pop. Limewater is the test for carbon dioxide and a glowing splint relights in oxygen.' },
        { q: 'Dilute hydrochloric acid on magnesium. Which salt is formed?',
          options: ['Magnesium chloride', 'Magnesium sulfate', 'Magnesium oxide', 'Magnesium carbonate'],
          why: 'Hydrochloric acid always makes chlorides: magnesium + hydrochloric acid → magnesium chloride + hydrogen.' }],
      exam: 'Grade 8 (g8s-acids-003, -012): the gas from an acid and a metal is hydrogen, and a lighted splint gives a squeaky pop.' },
    { id: 'g8_marble', grades: [8], chapter: 'g8s-acids', icon: '🪨',
      title: 'Which gas comes off marble chips?',
      aim: 'Dilute hydrochloric acid is in the tube. Drop in marble chips - a carbonate - and test the gas they give off.',
      setup: ['goggles', 'liquid:hcl'],
      predict: { q: 'Marble chips in acid. Which gas fizzes off?', answer: 'co2',
        options: [{ id: 'co2', label: 'Carbon dioxide' }, { id: 'h2', label: 'Hydrogen' }, { id: 'o2', label: 'Oxygen' }, { id: 'none', label: 'No gas at all' }] },
      steps: [
        { ask: 'Marble is calcium CARBONATE. Which solid is it?', on: 'solid:marble', options: ['solid:marble', 'solid:magnesium'],
          wrong: { 'solid:magnesium': 'Magnesium is a metal, not a carbonate - with acid it gives hydrogen. Marble chips are the carbonate.' } },
        { on: 'observe', say: 'Watch the tube. What happens to the chips?' },
        { ask: 'Test the gas. Which test is for carbon dioxide?', on: 'lime', options: ['lime', 'pop'],
          wrong: { pop: 'A lighted splint tests for hydrogen. In this gas the flame just goes out - that says NOT hydrogen, but not what it is. Limewater is the test for carbon dioxide.' } },
      ],
      see: { saw: 'The marble chips fizzed in the acid. The limewater turned milky: the gas is carbon dioxide.',
             learn: 'Acid + carbonate → salt + water + carbon dioxide. Carbon dioxide turns limewater milky - that is the test for it.' },
      check: [
        { q: 'What is formed when an acid reacts with a carbonate?',
          options: ['Salt + water + carbon dioxide', 'Salt + hydrogen', 'Salt + water', 'Water + oxygen'],
          why: 'A carbonate gives carbon dioxide as well as the salt and water. A metal gives hydrogen instead; a base gives just salt and water.' },
        { q: 'Which test shows a gas is carbon dioxide?',
          options: ['Limewater turns milky', 'A lighted splint gives a squeaky pop', 'A glowing splint relights', 'Red litmus turns blue'],
          why: 'Carbon dioxide turns limewater milky. The squeaky pop is hydrogen; a glowing splint relighting is oxygen.' },
        'g8_antacid:1'],
      exam: 'Grade 8 (g8s-acids-004): what is formed when an acid reacts with a carbonate? Salt + water + carbon dioxide.' },
  ];
  ...
  return { LIQUIDS, METALS, MEDIUM_NAMES, REACTIONS, reaction, fizzRating, FIZZ_WORDS, NEUTRAL,
           ...
           phOf, pHMix, litmus, IND8, RESULTS8, FACTS8, EXPERIMENTS };
})();
