...
//
//  ⚠ examWeight is DERIVED FROM MEASURED MARKS, not guessed - the 2025 paper's
//    marks for that content area × 0.4, largest-remainder to a total of exactly
//    40. g9m-surface-area scored 0 marks in 2025 and is PINNED at 2 anyway: one
//    year is not a weighting, and it is a full syllabus content area.
//
//  ⚠⚠ CORRECTED 2026-09-08: these were marks HALVED (× 0.5), which summed to 52.
//    A weight is a chapter's share of a FORTY-question paper, so the multiplier
//    is 0.4, not 0.5. The old sum was not merely untidy - it silently gutted the
//    top of the pack. assembleExamPaper() reconciles an over-weight pack with
//    `while (total > count) { const i = weights.findIndex(w => w.n > 1); … }`,
//    which always decrements the FIRST chapter in list order that is still above
//    1. At a sum of 52 it had to shed 12 questions, so it walked down from the
//    top: g9m-number-revision, the heaviest chapter in the pack at weight 8 and
//    the reason the chapter exists at all (~30 of the paper's 100 marks are
//    Grade 7-8 arithmetic), was dealt ONE question per exam instead of eight.
//    Finance lost 2 of 3, geometry-revision and indices the rest. Every chapter
//    from g9m-trigonometry onwards got exactly what it asked for.
//    ⚠ scripts/test-exam-paper-shape.js did NOT catch it: it asserts the paper
//    totals 40, which it did throughout. The distortion was in the distribution,
//    not the count. At the corrected weights the reconciliation loop shaves
//    nothing at all and every chapter is dealt precisely its weight.
//
...
registerSubject({
  ...
  chapters: [
    // ── NUMBERS ─────────────────────────────────────────────────────
    { id: 'g9m-number-revision', name: 'Number Revision (Grades 7-8)',    icon: '🧮', examWeight: 6,
    ...
    { id: 'g9m-indices',         name: 'Indices',                         icon: '⬆️', examWeight: 2,
    ...
    { id: 'g9m-finance',         name: 'Personal & Household Finance',    icon: '💰', examWeight: 2,
    ...
    // ── GEOMETRY ────────────────────────────────────────────────────
    { id: 'g9m-geometry-revision', name: 'Geometry Revision (Grades 7-8)', icon: '📏', examWeight: 3,
    ...
    { id: 'g9m-coordinates',     name: 'Coordinates & Straight Lines',    icon: '📈', examWeight: 3,
    ...
    { id: 'g9m-vectors',         name: 'Vectors & Translation',           icon: '➡️', examWeight: 3,
    ...
    { id: 'g9m-volume',          name: 'Volume',                          icon: '📦', examWeight: 3,
    ...
    // ── ALGEBRA ─────────────────────────────────────────────────────
    { id: 'g9m-expressions',     name: 'Algebraic Expressions',           icon: '🔡', examWeight: 2,
    ...
    { id: 'g9m-quadratics',      name: 'Quadratics',                      icon: '🌊', examWeight: 2,
    ...
    { id: 'g9m-inequalities',    name: 'Inequalities',                    icon: '📉', examWeight: 1,
    ...
    { id: 'g9m-simultaneous',    name: 'Simultaneous Equations',          icon: '⚖️', examWeight: 1,
    ...
    { id: 'g9m-statistics',      name: 'Statistics',                      icon: '📊', examWeight: 1,
    ...
    { id: 'g9m-probability',     name: 'Probability',                     icon: '🎲', examWeight: 2,
