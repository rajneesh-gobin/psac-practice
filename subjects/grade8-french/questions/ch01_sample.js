'use strict';
// Grade 8 Français - PLACEHOLDER. Delete this file when real questions land.
// IDs format: g8fr-samp-NNN
//
// This exists so the questions/ directory is not empty and the file shape is
// copyable. The pack is comingSoon: true, so nothing here is ever served to a
// child: QuestionLoader skips comingSoon packs and assembleExamPaper never sees
// the chapter.

STATIC_QUESTIONS.push(

  makeMCQ({ id:'g8fr-samp-001', chapterId:'g8fr-co', difficulty:1,
    question:'Placeholder - this pack has no real questions yet.',
    options:['A','B','C','D'],
    answer:'A',
    hint:'Replace this file with real Grade 8 Français questions.',
    explanation:'Placeholder question for the Grade 8 Français pack.' })

);
