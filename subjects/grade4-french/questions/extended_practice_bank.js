...
(function () {
  ...
  nouns.forEach(([word, definite, indefinite, plural], i) => {
    ...
    q({ chapterId:'g4fr-noms', subsection:'genre', difficulty:2, question:`Dans « ${indefinite} ${word} », le nom est-il masculin ou féminin ?`, options:choices(gender, ['masculin','féminin','pluriel seulement','un verbe']), answer:gender, hint:`L’article indéfini « ${indefinite} » indique le genre dans ce groupe nominal.`, explanation:`Dans <b>${indefinite} ${word}</b>, le nom est <b>${gender}</b>. « un » est masculin et « une » est féminin : ici, c’est l’article qui donne le genre.` });
  });
  ...
  readers.forEach(([text, question, answer, kind, tip], ri) => {
    for (let r = 0; r < 10; r++) q({ chapterId:'g4fr-lecture', subsection:r % 3 === 0 ? 'reperage' : r % 3 === 1 ? 'idee_principale' : 'inference', difficulty:r % 3 === 0 ? 1 : r % 3 === 1 ? 2 : 3, question:`<div style="background:#f8fafc;border-left:4px solid #6366f1;padding:10px;border-radius:6px;color:#0f172a"><b>Lis le texte.</b><br><br>${text} ${readerDetails[r]}</div><br>${question}`, options:choices(answer, r % 2 ? ['À Port Louis','Avec son professeur','Le matin suivant','Parce qu’il fait chaud'] : ['À Port Louis','Avec son professeur','Le matin suivant','Parce qu’il fait chaud']), answer, hint:`Pour une question « ${kind} », relis le texte et cherche ${tip}.`, explanation:`La réponse est <b>${answer}</b>. Elle est donnée directement ou peut être comprise grâce au texte.` });
  });
