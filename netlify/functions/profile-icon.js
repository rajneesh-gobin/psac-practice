...
exports.handler = async event => {
  ...
  if (!/^[a-f0-9]{32}$/i.test(key)) return {statusCode:400,body:'Invalid icon.'};
  const teacher = q.type === 'teacher';
  const name = clean(q.name, 24) || (teacher ? 'Teacher' : 'Student');
  ...
  return {statusCode:200,headers:{'Content-Type':'image/svg+xml; charset=utf-8','Cache-Control':'public, max-age=86400','X-Content-Type-Options':'nosniff'},body:svg};
};
