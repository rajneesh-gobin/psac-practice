...
export default async function handler(request, env) {
  ...
  if (!UUID_RE.test(materialId)) {
    return new Response(JSON.stringify({ ok: false, error: 'bad_material' }), { status: 400, headers: HEADERS });
  }
  ...
  try {
    const r = await fetch(url + '/rest/v1/rpc/guest_mark_material', {
      ...
      body: JSON.stringify({ p_code: code, p_name: name, p_token: token, p_material_id: materialId, p_done: done }),
