...
--    added to production since 2026-09-25 survives; and it overwrites function,
...
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS id bigint GENERATED BY DEFAULT AS IDENTITY;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS family_id uuid;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS actor_kind text;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS actor_id uuid;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS as_student uuid;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS kind text;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS ref text;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS meta jsonb;
ALTER TABLE public.activity_events ADD COLUMN IF NOT EXISTS created_at timestamp with time zone DEFAULT now();
ALTER TABLE public.activity_events ALTER COLUMN id SET NOT NULL;
ALTER TABLE public.activity_events ALTER COLUMN actor_kind SET NOT NULL;
ALTER TABLE public.activity_events ALTER COLUMN actor_id SET NOT NULL;
ALTER TABLE public.activity_events ALTER COLUMN kind SET NOT NULL;
ALTER TABLE public.activity_events ALTER COLUMN created_at SET NOT NULL;
...
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS id bigint GENERATED BY DEFAULT AS IDENTITY;
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS admin_id uuid;
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS target_user uuid;
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS target_student uuid;
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS action text;
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS detail jsonb DEFAULT '{}'::jsonb;
ALTER TABLE public.admin_actions ADD COLUMN IF NOT EXISTS created_at timestamp with time zone DEFAULT now();
ALTER TABLE public.admin_actions ALTER COLUMN id SET NOT NULL;
ALTER TABLE public.admin_actions ALTER COLUMN action SET NOT NULL;
ALTER TABLE public.admin_actions ALTER COLUMN detail SET NOT NULL;
ALTER TABLE public.admin_actions ALTER COLUMN created_at SET NOT NULL;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'activity_events_pkey'
                    AND conrelid = 'activity_events'::regclass) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'admin_actions_pkey'
                    AND conrelid = 'admin_actions'::regclass) THEN
    ALTER TABLE admin_actions ADD CONSTRAINT admin_actions_pkey PRIMARY KEY (id);
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'activity_events_actor_kind_check'
                    AND conrelid = 'activity_events'::regclass) THEN
    ALTER TABLE activity_events ADD CONSTRAINT activity_events_actor_kind_check CHECK ((actor_kind = ANY (ARRAY['parent'::text, 'student'::text])));
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'activity_events_kind_check'
                    AND conrelid = 'activity_events'::regclass) THEN
    ALTER TABLE activity_events ADD CONSTRAINT activity_events_kind_check CHECK (((char_length(kind) >= 1) AND (char_length(kind) <= 40)));
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'activity_events_ref_check'
                    AND conrelid = 'activity_events'::regclass) THEN
    ALTER TABLE activity_events ADD CONSTRAINT activity_events_ref_check CHECK ((char_length(ref) <= 120));
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'activity_events_as_student_fkey'
                    AND conrelid = 'activity_events'::regclass) THEN
    ALTER TABLE activity_events ADD CONSTRAINT activity_events_as_student_fkey FOREIGN KEY (as_student) REFERENCES students(id) ON DELETE CASCADE;
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
                  WHERE conname = 'activity_events_family_id_fkey'
                    AND conrelid = 'activity_events'::regclass) THEN
    ALTER TABLE activity_events ADD CONSTRAINT activity_events_family_id_fkey FOREIGN KEY (family_id) REFERENCES families(id) ON DELETE CASCADE;
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
...
-- ═══ 4 · FUNCTIONS ════════════════════════════════════════════════════════════
-- 147 functions, verbatim from pg_get_functiondef().
--
...
-- ── admin_patch_student_settings(p_student uuid, p_patch jsonb, p_reason text)
...
-- ── prune_activity_events(p_keep integer, p_days integer)
CREATE OR REPLACE FUNCTION public.prune_activity_events(p_keep integer DEFAULT 500, p_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'extensions'
...
-- constraint itself. 94 standalone indexes.
CREATE INDEX IF NOT EXISTS activity_events_actor_idx ON public.activity_events USING btree (actor_id, created_at DESC);
CREATE INDEX IF NOT EXISTS activity_events_created_idx ON public.activity_events USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS activity_events_family_idx ON public.activity_events USING btree (family_id, created_at DESC);
...
CREATE TRIGGER activity_events_stamp BEFORE INSERT ON public.activity_events FOR EACH ROW EXECUTE FUNCTION activity_events_stamp();
...
--
-- RLS is enabled on all 63 public tables.
...
CREATE POLICY activity_insert_self ON public.activity_events
  ...
  WITH CHECK ((((actor_kind = 'student'::text) AND (actor_id = current_student_id())) OR ((actor_kind = 'parent'::text) AND (actor_id = auth.uid()))));
...
-- ── table grants
GRANT INSERT, MAINTAIN, REFERENCES, TRIGGER ON public.activity_events TO anon;
GRANT INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER ON public.activity_events TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON public.activity_events TO service_role;
GRANT SELECT ON public.admin_actions TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON public.admin_actions TO service_role;
...
GRANT EXECUTE ON FUNCTION public.activity_events_stamp() TO service_role;
...
GRANT EXECUTE ON FUNCTION public.prune_activity_events(p_keep integer, p_days integer) TO authenticated, service_role;
