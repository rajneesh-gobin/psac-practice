...
const added = bank.filter(q => q.id.startsWith('g6eng-joinx-'));
const sections = ['relative','cause','contrast','time','purpose_result'];
const norm = s => s.replace(/\s+/g, ' ').trim().toLowerCase();
const prompts = new Map();
for (const q of bank) prompts.set(norm(q.question), (prompts.get(norm(q.question)) || 0) + 1);
assert.equal(added.length, 50);
assert.equal(new Set(added.map(q => q.id)).size, 50);
...
for (const section of sections) {
  assert.equal(added.filter(q => q.subsection === section).length, 10);
  const count = bank.filter(q => q.chapterId === 'g6eng-enr-joining' && q.subsection === section).length;
  assert(count >= 20, `${section}: ${count}`);
  console.log(`Grade 6 English joining/${section}: ${count} questions`);
}
console.log('50 sentence-joining additions passed coverage and structural checks.');
