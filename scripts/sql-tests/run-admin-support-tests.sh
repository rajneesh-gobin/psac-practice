#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
#  Validates migrations/20260911_admin_support_panel.sql by building
...
#    4. SEED          two admins (one super), three parents, three children
#    5. BEHAVIOUR     admin-support-assert.sql
#
...
#
#  Usage: scripts/sql-tests/run-admin-support-tests.sh
# ═══════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
ROOT="$(cd "$HERE/../.." && pwd)"
C=psac-admin-support-test
SCHEMA="$ROOT/supabase-schema.sql"
MIG="$ROOT/migrations/20260911_admin_support_panel.sql"
...
docker cp "$(hostpath "$HERE/admin-support-seed.sql")"          "$C":/seed.sql
docker cp "$(hostpath "$HERE/admin-support-assert.sql")"        "$C":/assert.sql
...
run() {          # run <label> <file> [show]
  ...
  out="$(printf '%s\n' "$out" | sed 's/^psql:[^ ]*: //;s/^NOTICE:  //' \
          | grep -vE 'already exists, skipping|^$|^t$|^f$' || true)"
  if [ $rc -ne 0 ]; then
...
run "20260911_admin_support_panel.sql" /migration.sql
...
echo
echo "4. seed"
run "admin-support-seed.sql" /seed.sql

echo
echo "5. behaviour"
run "admin-support-assert.sql" /assert.sql show

echo
if [ "$FINDINGS" -gt 0 ]; then
  echo "$FINDINGS SECURITY FINDING(S)."
  exit 1
fi
...
echo "Admin support panel: schema builds, migration is idempotent, only a super admin"
echo "moves admin rights or a child's settings, and every change is recorded."
