...
const added = bank.filter(q => q.id.startsWith('g6eng-fd-'));
...
for (const [section, countAdded] of [['formal_letter',17], ['descriptive',19]]) {
  assert.equal(added.filter(q => q.subsection === section).length, countAdded);
  const count = bank.filter(q => q.chapterId === 'g6eng-writing' && q.subsection === section).length;
  assert(count >= 20, `${section}: ${count}`);
  console.log(`Grade 6 English ${section}: ${count} questions`);
}
console.log('36 formal-letter/descriptive additions passed coverage and structural checks.');
